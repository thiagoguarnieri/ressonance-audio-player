...
Public Const SPECWIDTH As Long = 332 ' display width
Public Const SPECHEIGHT As Long = 63 ' height (changing requires palette adjustments too)
...
Public Sub UpdateSpectrum(ByVal uTimerID As Long, ByVal uMsg As Long, ByVal dwUser As Long, ByVal dw1 As Long, ByVal dw2 As Long, stream As Long)
    ...
    If (specmode = 3) Then ' waveform
        ...
        Call BASS_ChannelGetInfo(stream, ci) ' get number of channels
        ReDim buf(ci.chans * SPECWIDTH * LenB(buf(0))) As Single  ' allocate buffer for data

        Call BASS_ChannelGetData(stream, buf(0), (ci.chans * SPECWIDTH * LenB(buf(0))) Or BASS_DATA_FLOAT)  ' get the sample data (floating-point to avoid 8 & 16 bit processing)
    ...
    Else
        ...
        Call BASS_ChannelGetData(stream, fft(0), BASS_DATA_FFT2048)
...
#End If
    ...
    Call SetDIBitsToDevice(Form1.vuMeter.hDC, 0, 0, SPECWIDTH, SPECHEIGHT, 0, 0, 0, SPECHEIGHT, specbuf(0), bh, 0)
    End If
End Sub
